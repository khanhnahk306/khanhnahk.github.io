...
if (type) {
    if (type == "onLab") {
        document.title("Khóa học phognf lab")
        courseList = coourses.filter(c => c.type == "onLab");
    } else if (type == "online") {
        courseList = coourses.filter(c => c.type == "online");
    } else {
...
const filterCoursesByTopics = topic => {
    let coursesFilter = courses.filter(c => c.topics.includes(topic));
